local function set_terminal_bg(hex)
  ...
  if term:match("iTerm") or term:match("kitty") then
    local r = tonumber(hex:sub(2,3),16)
    local g = tonumber(hex:sub(4,5),16)
    local b = tonumber(hex:sub(6,7),16)
    io.write(string.format("\27]11;rgb:%02x/%02x/%02x\a", r,g,b))
    io.flush()
  end
end
...
vim.api.nvim_create_augroup("TerminalBackgroundSync", { clear = true })

vim.api.nvim_create_autocmd({ "VimEnter", "ColorScheme" }, {
  group = "TerminalBackgroundSync",
  callback = apply_nvim_bg_to_terminal,
})

vim.api.nvim_create_autocmd("VimLeavePre", {
  group = "TerminalBackgroundSync",
  callback = restore_terminal_bg,
})
